...
def validate_import_upload(uploaded) -> dict[str, bool]:
    ...
    results["size"] = size <= MAX_IMPORT_FILE_BYTES

    if results["format"] and results["size"]:
        try:
            results["merge_cell"] = not _file_has_merged_cells(uploaded)
            uploaded.seek(0)
            raw_df = read_import_file(uploaded)
            missing = get_missing_dashboard_columns(raw_df)
            results["structure"] = len(missing) == 0
            results["required_filled"] = _required_columns_filled(raw_df) if results["structure"] else False
        except Exception:
            pass
        finally:
            uploaded.seek(0)
